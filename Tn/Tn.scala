...
lazy val cmd = {
	...
	try { List(sys.props("script")) find((x:String) => {x.matches(".*[a-zA-Z]{1}.*")}) get } catch {
	...
	def read(arg:String) :Unit = {
		...
		arg.head match {
	      	case a if matched contains a => sys.error("duplicated argument: " + a)
				...
				case 'v' => matched::='v';matched::='q'; if (arg.tail.length > 0) read(arg.tail)
								debug=true
				case 'q' => matched::='q';matched::='v'; if (arg.tail.length > 0) read(arg.tail)
								quiet=true
				case 'u' => matched::='u'; if (arg.tail.length > 0) read(arg.tail)
								usage=true
				case 'b' => matched::='b'; blackDigs=arg.tail
				case 'l' => matched::='l'; if (arg.tail.length > 0) read(arg.tail)
								lister=true
...
// main routine
	try {
		args.foreach( (x) => { if (x.startsWith("-")) read(x.tail) } )
		if (n < 0) {
...
val padL = (end toString rad).length 
val padR = (( end * (end+1) )  /2).toString(rad).length
...
def go():BigInt = {
	...
	if (lister) {
		val s = n toString rad
		val l = s.length
		val pad = " " * (padL - l)	
		val x = res.toString(rad)
		...
		val x =  ((res % pow(rad, pos + 1).toInt) / pow(rad, pos).toInt).toString(rad)
